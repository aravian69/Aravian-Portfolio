...
const aspectPath  = path.join(__dir, '..', 'lib', 'aspectRatio.ts');
...
async function sampleUrl(url) {
  ...
  // Real aspect ratio (w/h) straight from the source — the thumbnail is the
  // actual video frame, so this is the true content ratio. Clamped to a sane
  // range so an odd banner can't produce an absurd card.
  const meta = await sharp(buf).metadata();
  let aspect = null;
  if (meta.width && meta.height) {
    aspect = Math.round(Math.min(2.4, Math.max(0.42, meta.width / meta.height)) * 1000) / 1000;
  }
  ...
  const swatch = weight === 0 ? null : rgbToHsl(r / weight, g / weight, b / weight);
  return { swatch, aspect };
}
...
async function main() {
  ...
  for (let i = 0; i < items.length; i += CONC) {
    ...
    await Promise.all(slice.map(async ({ id, url }) => {
      ...
      try {
        const { swatch, aspect } = await sampleUrl(url);
        const key = sortKey(swatch);
        order[id] = Math.round(key * 10) / 10;
        if (aspect) aspects[id] = aspect;
...
`;
  ...
  const aspectBody = Object.entries(aspects)
    .map(([id, a]) => `  ${JSON.stringify(id)}: ${a},`)
    .join('\n');
  const aspectFile = `// AUTO-GENERATED by scripts/compute-hues.mjs — do not edit by hand.
...
`;
  fs.writeFileSync(aspectPath, aspectFile, 'utf8');
  console.log(`\n✅  Wrote lib/hueOrder.ts (${ok} sampled, ${miss} unsortable) + lib/aspectRatio.ts (${Object.keys(aspects).length}).\n`);
}
